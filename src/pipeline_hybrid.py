import argparse
import random
import re
import time
from datetime import datetime
from . import script, voice, captions, visuals_web, visuals_ai, assemble_ai, upload, state
from .config import CONFIG, OUTPUT_DIR

def slug(s: str) -> str:
    return re.sub(r"[^a-z0-9]+", "-", s.lower()).strip("-")[:60] or "anime-short"

def _log(msg: str):
    print(f"[{datetime.now().strftime('%H:%M:%S')}] {msg}")

_AI_STYLES = [
    "Studio Ghibli style", "Makoto Shinkai style", "KyoAni style",
    "Ufotable style", "anime art style", "manga panel style",
    "vintage anime 80s style", "A-1 Pictures style",
]

def run_once(publish_at: str | None = None,
             upload_to_youtube: bool = True) -> dict:
    _log("1/7 Generating script with LLM")
    data = script.generate()
    _log(f"    topic: {data['topic']} ({len(data['scenes'])} scenes)")

    stamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    work = OUTPUT_DIR / f"{stamp}_{slug(data['topic'])}"
    work.mkdir(parents=True, exist_ok=True)

    _log("2/7 Synthesizing voiceover with Edge TTS")
    voice_mp3 = voice.synth(data["full_text"], work / "voice.mp3")
    _log(f"    voice saved ({voice_mp3.stat().st_size/1024:.0f} KB)")

    _log("3/7 Transcribing for word-level captions (Faster-Whisper)")
    _log("    loading model (first run downloads)...")
    t0 = time.time()
    words = captions.transcribe_words(voice_mp3)
    _log(f"    {len(words)} words in {time.time()-t0:.1f}s")

    vcfg = CONFIG.get("visuals", {})
    source = vcfg.get("source", "hybrid")
    mix = vcfg.get("hybrid_mix", 0.5)

    _log(f"4/7 Fetching visuals (source={source}, mix={mix})")

    image_paths = []
    for i, scene in enumerate(data["scenes"]):
        prompt = scene.get("visual_query", "abstract background")
        img_path = work / "images" / f"scene_{i:02d}.jpg"
        img_path.parent.mkdir(parents=True, exist_ok=True)

        use_web = random.random() < mix if source == "hybrid" else (source == "web")

        if use_web:
            _log(f"    scene {i+1}/{len(data['scenes'])} [WEB]: \"{prompt}\"")
            t1 = time.time()
            web_dir = work / "broll_web"
            clips = visuals_web.fetch_for_scenes([scene], web_dir)
            if clips and clips[0].exists() and clips[0].stat().st_size > 5000:
                image_paths.append(clips[0])
                _log(f"      done in {time.time()-t1:.0f}s")
                continue
            _log(f"      web failed, fallback to AI")

        style = random.choice(_AI_STYLES)
        rich_prompt = f"{prompt}, {style}, cinematic, high quality, detailed"
        _log(f"    scene {i+1}/{len(data['scenes'])} [AI]: \"{prompt}\" ({style})")
        t1 = time.time()
        visuals_ai.generate(rich_prompt, img_path)
        _log(f"      done in {time.time()-t1:.0f}s")
        image_paths.append(img_path)

    _log("5/7 Writing caption file")
    ass_path = captions.write_ass(words, work / "captions.ass",
                                  CONFIG["video"]["width"], CONFIG["video"]["height"])

    _log("6/7 Assembling slideshow video with ffmpeg")
    t0 = time.time()
    final = assemble_ai.build(
        image_paths=image_paths,
        voice_audio=voice_mp3,
        captions_ass=ass_path,
        words=words,
        scenes=data["scenes"],
        out_path=work / "final.mp4",
        work_dir=work / "ffmpeg",
    )
    dur = time.time() - t0
    sz = final.stat().st_size / (1024 * 1024)
    _log(f"    final: {final.name} ({sz:.0f} MB, {dur:.0f}s render)")

    video_id = None
    if upload_to_youtube:
        _log("7/7 Uploading to YouTube")
        video_id = upload.upload_video(
            video_path=final,
            title=data["title"],
            description=data["description"],
            tags=data["tags"],
            publish_at=publish_at,
        )
        _log(f"    uploaded: https://youtube.com/shorts/{video_id}")

    state.add_topic(data["topic"])
    state.add_published({
        "ts": stamp, "topic": data["topic"], "title": data["title"],
        "path": str(final), "video_id": video_id, "publish_at": publish_at,
    })
    return {"video_id": video_id, "path": str(final), "topic": data["topic"]}

def main():
    p = argparse.ArgumentParser(description="Anime/Manga hybrid pipeline (web + AI visuals)")
    p.add_argument("--no-upload", action="store_true")
    p.add_argument("--publish-at", default=None,
                   help="ISO8601 UTC timestamp for scheduled publish")
    p.add_argument("--source", default=None, choices=["web", "ai", "hybrid"],
                   help="Override visual source")
    args = p.parse_args()

    if args.source:
        CONFIG.setdefault("visuals", {})["source"] = args.source

    run_once(publish_at=args.publish_at, upload_to_youtube=not args.no_upload)
    print("\n" + "-" * 60)
    print("Done!")
    print("-" * 60)

if __name__ == "__main__":
    main()
